        ...
        for(auto i = 0; i < Particles::max_particle_count; i++) {
            ...
            if(toGrid) {
                ...
                if(d > 0.0) {
                    ...
                    auto picV =
                        (valid0 * d0 * vel(bl) + valid1 * d1 * vel(br) + valid2 * d2 * vel(tr) + valid3 * d3 * vel(tl)) / d;
    ...
    for(auto iter = 0; iter < numIters; iter++) {
        ...
        for(auto i = 1; i < m_width - 1; i++) {
            for(auto j = 1; j < m_height - 1; j++) {
                ...
                auto div = velocities[right].x - velocities[center].x + velocities[top].y - velocities[center].y;
...
void Fluid::draw(AABB area, sf::RenderTarget &window, std::unordered_map<eCellTypes, Color> color_table)
{
    sf::Image img;
